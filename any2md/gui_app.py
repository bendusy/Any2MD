...
class PreScanner(QThread):
    files_found = pyqtSignal(list) # list[FileItemData]
    ...
    def run(self):
        ...
        valid_exts = {'.pdf', '.doc', '.docx', '.ppt', '.pptx', '.xls', '.xlsx', '.html', '.htm', '.txt', '.rtf', '.md'}
...
class ConvertWorker(QThread):
    ...
    def __init__(self, items: List[FileItemData], output_path: Path, merge: bool, merge_name: str):
        super().__init__()
        self.items = items
            ...
            for i, item in enumerate(self.items):
                ...
                try:
                    ...
                    if res.success:
                        ...
                        self.file_finished.emit(str(item.path), False, res.error or "Unknown error")
                    ...
                    results.append(ConvertResult(success=False, input_path=item.path, output_path=None, error=str(e)))
...
class ResultDialog(QDialog):
    def __init__(self, parent, results: List[ConvertResult]):
        ...
        summary.setStyleSheet(f"font-size: 16px; font-weight: 600; color: {'red' if fail_count > 0 else MorningTheme.SUCCESS}")
...
class InstructionStep(QWidget):
    def __init__(self, number: str, text: str):
        ...
        text_label.setStyleSheet(f"color: {MorningTheme.TEXT_SECONDARY}; font-size: 14px;")
...
class DropZone(QFrame):
    ...
    def __init__(self):
        ...
        self.icon_label.setStyleSheet(f"font-size: 72px; color: {MorningTheme.BORDER_LIGHT};")
        ...
        self.main_text.setStyleSheet(f"font-size: 20px; font-weight: 600; color: {MorningTheme.TEXT_PRIMARY};")
        ...
        support_text.setStyleSheet(f"color: {MorningTheme.TEXT_SECONDARY}; font-size: 13px;")
    ...
    def _set_active(self, active: bool):
        if active:
            ...
            self.icon_label.setStyleSheet(f"font-size: 80px; color: {MorningTheme.ACCENT_PRIMARY};")
            self.main_text.setText("释放以添加")
            self.main_text.setStyleSheet(f"font-size: 22px; font-weight: 700; color: {MorningTheme.ACCENT_PRIMARY};")
        else:
...
class ZenWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.flp_title.setStyleSheet(f"font-size: 18px; font-weight: 600; color: {MorningTheme.TEXT_PRIMARY};")
        ...
        self.scan_loading.setStyleSheet(f"font-size: 16px; color: {MorningTheme.TEXT_SECONDARY};")
        ...
        lbl_help_merge = QLabel("将所有文档整合到一个 Markdown 文件中，非常适合上传给 Claude 或 ChatGPT 进行分析。")
        ...
        self.status_label.setStyleSheet(f"color: {MorningTheme.TEXT_SECONDARY}; font-size: 13px;")
    ...
    def on_finished_all(self, results):
        ...
        if fail_count == 0:
            ...
            self.status_label.setStyleSheet(f"color: {MorningTheme.SUCCESS}; font-weight: 500;")
            
            reply = QMessageBox.question(self, "完成", f"成功处理所有 {len(results)} 个文件。\n是否打开输出目录？", 
            ...
            self.status_label.setStyleSheet(f"color: {MorningTheme.ERROR}; font-weight: 500;")
